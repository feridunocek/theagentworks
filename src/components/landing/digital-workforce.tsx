...
export function DigitalWorkforce() {
    ...
    return (
        <section id="birimler" className="py-16 md:py-24 relative overflow-hidden">
            {/* Background Decorations */}
            <div className="absolute top-1/4 left-0 w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-[var(--color-neon-purple)]/5 rounded-full blur-[80px] md:blur-[100px] -z-10" />
            <div className="absolute bottom-1/4 right-0 w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-[var(--color-neon-cyan)]/5 rounded-full blur-[80px] md:blur-[100px] -z-10" />
            ...
            <div className="container px-4 md:px-6 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    {units.map((unit, index) => (
                        ...
                        >
                            ...
                            <div className="relative h-full p-6 md:p-8 rounded-2xl border border-white/5 bg-white/[0.02] backdrop-blur-md overflow-hidden hover:border-[var(--color-neon-purple)]/30 hover:bg-white/[0.04] transition-all duration-500 hover:shadow-[0_0_30px_-10px_rgba(168,85,247,0.15)] flex flex-col">

                                {/* Header */}
                                <div className="flex items-center gap-4 mb-4 md:mb-6">
                                    <div className={`p-2 md:p-3 rounded-xl bg-white/5 border border-white/10 ${unit.color} group-hover:scale-110 transition-transform duration-300`}>
                                        <unit.icon size={20} className="md:w-6 md:h-6" />
                                    </div>
                                    <h3 className="text-lg md:text-xl font-bold text-white group-hover:text-[var(--color-neon-purple)] transition-colors">
                                        {unit.title}
                                    </h3>
                                </div>

                                {/* Features List */}
                                <ul className="space-y-3 md:space-y-4 grow">
                                    {unit.features.map((feature, i) => (
                                        <li key={i} className="flex items-start gap-3 text-sm text-gray-400 group-hover:text-gray-300 transition-colors">
                                            <div className="mt-0.5 md:mt-1 min-w-4 max-w-4 text-[var(--color-neon-cyan)]/70">
                                                <Check size={16} />
                                            </div>
                ...
                >
                    <a href="#iletisim" onClick={(e) => { e.preventDefault(); document.getElementById('iletisim')?.scrollIntoView({ behavior: 'smooth' }); }}>
                        <NeonButton variant="primary" glow className="w-full sm:w-auto px-8 py-5 md:py-6 text-base md:text-lg justify-center">
